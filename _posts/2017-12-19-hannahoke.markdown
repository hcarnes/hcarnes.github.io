...
Each instance of KaraokeBar is initialized with `@queue = []` to store singers as they sign up and `@song_history` to store each song that has already been performed. There is also an `attr_reader` for `:queue`, so that we can add singers to the queue and get access to the array of singers.
...
class KaraokeBar
  ...
  ```
As each person signs up, they provide their name, the song name, and an artist name. The `add_singer` method checks to see if a requested song has been played recently or if someone else in the queue has chosen that song. If the song is a duplicate, the hopeful singer will be asked to choose a new song. If they picked a new song, the singer's information gets added to the queue as a hash. They also receive a message about their status in the line.
  ...
  ```
The `on_deck` method let's the next person know they need to get ready to hit the stage. There is no turning back. The `@queue.first` is set to a new variable, so that we can access the hash of singer information and call the next singer by  name. 
